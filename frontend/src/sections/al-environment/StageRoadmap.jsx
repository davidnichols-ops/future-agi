...
const stageState = (status) => {
  ...
  return {
    ...
    understand: { done: Boolean(have.contract), sub: have.contract ? "written" : "" },
    build: { done: Boolean(have.world), sub: have.world ? `${have.sub_goals || 0} sub-goals` : "" },
...
const StageRoadmap = ({ status, onSelectStage }) => {
  ...
  return (
    <Stack direction="row" alignItems="flex-start" sx={{ flexGrow: 1, maxWidth: 660, mx: "auto" }}>
      {ALK_STAGES.map((stage, index) => {
        ...
        const tone = (() => {
          if (state.done) return doneGreen;
          if (current) return "text.primary";
        ...
        return (
          <Box
            ...
            type="button"
            disabled={blocked}
            title={blockedBecause || `open the ${stage.label.toLowerCase()} stage`}
          ...
          >
            ...
            >
              {index > 0 && (
                <Box aria-hidden sx={rail("left", behindDone)} />
              )}
              {index < last && (
                <Box aria-hidden sx={rail("right", state.done)} />
              )}
              <Box
                className="alk-dot"
                aria-hidden
                ...
                sx={{
                  ...
                  borderColor: state.done || current ? "currentColor" : "text.disabled",
                  bgcolor: current && !state.done ? "currentColor" : "background.paper",
                  color: "inherit",
                }}
              >
                {state.done ? "✓" : ""}
            ...
            <Typography
              variant="caption"
              noWrap
              sx={{ fontFamily: ALK_MONO, fontWeight: current ? 700 : 400, color: "inherit" }}
            >
              {stage.label}
            </Typography>
            <Typography
              variant="caption"
              noWrap
              sx={{ fontFamily: ALK_MONO, fontSize: 10, maxWidth: "100%", color: "text.disabled" }}
            >
