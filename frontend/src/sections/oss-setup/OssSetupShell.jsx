...
// Centered shell shared by the OSS setup steps. A single column on the app
// background with the Future AGI wordmark on top — no promo panel, so the setup
// content sits in the middle of the screen.
export default function OssSetupShell({ step, totalSteps, illustration, children }) {
  return (
    <Box
      sx={{
        width: "100%",
    ...
    >
      ...
      >
        ...
        {/* Illustration (vertical hero by default; steps may override) */}
        <Stack alignItems="center" sx={{ mb: 3, mt: -1, width: "100%" }}>
          {illustration || <BlueprintSpaceship size={150} />}
        </Stack>

        {/* Step progress — dots stepper (matches the signup onboarding) */}
        {typeof step === "number" && (
