...
QueryStatus = Literal["complete", "degraded"]
...
# Exact-key discovery may continue through a small number of disjoint
# candidate pages when the first raw matches replay to cleared/tombstoned
# latest state. Five pages preserve the existing one-page-per-horizon-band
# budget and one extra page closes the common "live match is candidate 513"
# hole without turning the generic key inventory into an open-ended scan.
ATTRIBUTE_READ_TARGETED_CANDIDATE_PAGE_LIMIT = 6
# Value discovery is exact whenever every adaptive band exhausts inside this
# finite continuation contract.  Walk the bands round-robin so a dense recent
# week cannot hide values in older bands, while still allowing candidate 513+
# (including a live row behind stale physical versions) to be replayed.  The
# six-second wall deadline remains the tighter production bound; this page cap
# is the deterministic ceiling for fast/mocked executors.
...
class AttributeReadSelector:
    ...
    adaptive candidate and latest-state replay queries. Generic key browse is
    at most ten queries, exact-key discovery at most twelve, and value reads at
    most thirty under their fair round-robin continuation cap. Reusing a
        ...
        *,
        ...
        try:
            ...
            page = self._executor.execute(
                query,
                params,
                timeout_ms=min(ATTRIBUTE_READ_QUERY_TIMEOUT_MS, remaining_ms),
                settings={
    ...
    def _candidate_ids(
        ...
        *,
        ...
        query_settings: dict[str, Any] | None = None
        if stratified:
            candidate_sql = _STRATIFIED_CANDIDATE_SQL
        ...
        *,
        ...
        if exact_key is None:
            ...
            if self._reads_json_overflow:
                predicate += " OR attributes_extra NOT IN ('', '{}', 'null')"
        else:
            predicate = (
                "mapContains(attrs_string, %(attribute_key)s) "
            ...
            if self._reads_json_overflow:
                predicate += " OR JSONHas(attributes_extra, %(attribute_key)s)"
        ...
        for segment in windows:
            ...
            while True:
                ...
                try:
                    ...
                    if not latest_keys or not is_read_budget_error(exc):
                        raise
                ...
                for row in rows:
                    ...
                    if not self._row_is_active_in_window(
                    ...
                    if exact_key is not None:
                        ...
                        if (
                            ...
                            truncated = True
                        latest_keys[identity] = (
                            {exact_key: decoded[0]} if decoded is not None else {}
                        )
                    else:
                ...
                if exact_key is None:
                    ...
                    if segment_truncated:
                        candidate_scan_halted = True
                    break
                if any(latest_keys.values()):
                    # The key exists; as before, discovery need not count every
                    # occurrence. Keep degradation visible if this page itself
                    # proves there are unseen candidates.
                    truncated = truncated or segment_truncated
                    break
            ...
            if budget_exceeded or candidate_scan_halted:
                break
            if exact_key is not None and any(latest_keys.values()):
                break
            discovered_key_count = len(
                {key for keys in latest_keys.values() for key in keys}
            )
            if discovered_key_count > max_keys:
                truncated = True
                break
    ...
    def read_values(
        ...
        *,
        ...
        candidate_pages = 0
        while candidate_pages < ATTRIBUTE_READ_VALUE_TOTAL_CANDIDATE_PAGE_LIMIT and any(
            not state["complete"] for state in segment_states
        ):
            progressed = False
            for state in segment_states:
                ...
                try:
                    ...
                    if not latest_values or not is_read_budget_error(exc):
                        raise
                    budget_exceeded = True
                ...
                if not segment_truncated:
                    ...
                    # Defensive fail-closed guard: a truncation sentinel without
                    # any replayable identity cannot be continued safely.
                    truncated = True
                    state["complete"] = True
        ...
        if any(not state["complete"] for state in segment_states):
            truncated = True
